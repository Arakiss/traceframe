...
Traceframe records what an agent run did. The trace becomes useful as a *gate*:
...
| :-- | :-- | :-- | :-- |
| `verify` | Is the trace structurally sound (ordered, closed, unique ids)? | `traceframe verify --file <trace>` | `0` ok / non-zero broken |
| `policy-check` | Did the run avoid unresolved policy violations? | `traceframe policy-check --file <trace>` | `0` clean / non-zero violation |
...
shipped `examples/agent-run.traceframe` is exactly that case — it `verify`s fine
...
- name: Gate on the run trace
  run: |
    traceframe verify --file "$RUN_TRACE"
    traceframe policy-check --file "$RUN_TRACE"
```
...
written by `traceframe hook install` wiring, or by `traceframe run`).
...
trace=".traceframe/runs/session.traceframe"
[ -f "$trace" ] || exit 0          # no trace recorded; nothing to gate
traceframe verify --file "$trace" || { echo "trace failed verify" >&2; exit 1; }
traceframe policy-check --file "$trace" || { echo "trace failed policy-check" >&2; exit 1; }
```
