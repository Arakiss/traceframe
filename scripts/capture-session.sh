...
#
# This is a hands-on demo, not a CI test: it needs `traceframe` on PATH and an
...
#   2. wire its local hooks file from the `traceframe hook install` snippet,
...
# this demo can instead record a trace with the `traceframe run` wrapper.
#
# Each shell tool call fires the PostToolUse hook, which pipes the payload to
# `traceframe hook ingest --dir .traceframe/runs`; ingest derives the run id
# from the session id, so one trace file is written per session automatically.
set -eu

command -v traceframe >/dev/null || { echo "install traceframe first (cargo install --path .)" >&2; exit 1; }
...
cat > "$hooks_file" <<JSON
{
  "hooks": {
    "PostToolUse": [
      { "matcher": "Bash", "hooks": [ { "type": "command", "command": "traceframe hook ingest --source generic --dir $proj/.traceframe/runs" } ] }
    ]
  }
}
JSON
...
if [ -n "${AGENT_CMD:-}" ]; then
  ...
  echo "AGENT_CMD not set; recording a generic command trace with 'traceframe run' instead."
  echo "set AGENT_CMD to wire and capture a real agent harness session."
  traceframe run --file "$proj/.traceframe/runs/demo-session.traceframe" --run-id demo-session -- sh -c 'ls -la; wc -l notes.txt; cat notes.txt' || true
fi

trace="$(ls "$proj"/.traceframe/runs/*.traceframe 2>/dev/null | head -1 || true)"
[ -n "$trace" ] || { echo "no trace was produced — is the PostToolUse hook wired (or AGENT_CMD set)?" >&2; exit 1; }

echo ""; echo "captured trace: $trace"
traceframe verify  --file "$trace" --allow-open
traceframe summary --file "$trace"
traceframe render  --file "$trace" --html "$proj/report.html"
